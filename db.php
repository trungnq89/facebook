...
class DB {
    ...
    /**
     *  add quote before insert 
     *  author : Trung Nguyen Quang 
     * @param string $string
     * @return string
     */
    function quote($string = "test") {
    ...
    /**
     * check record exist, if not exit insert new once
     * author : Trung Nguyen Quang
     * 
     * @param type $field -- count field
     * @param type $table -- table name 
     * @param type $where  -- condition to find
     * @param type $return_first  -- if record exist , return first 
     * @param type $insert_if
     * @param type $data
     */
    function checkRecodeExist($field = "id", $table = "", $where = "", $return_first = true, $insert_if = true, $data = array()) {
        $insert_id = 0;
        $query = "SELECT " . $field . " FROM " . $table . " WHERE " . $where; 
        $this->query($query);   
        if ($this->num_rows() == 0) { 
            if ($insert_if ) { // insert if not exit
                $this->fields = $data;
                $this->insert($table);
                $insert_id = $this->insert_id(); 
            }
        } else { 
                $website = $this->get_records();
                $insert_id = $website[0][$field]; 
        }
        return $insert_id;
    }
